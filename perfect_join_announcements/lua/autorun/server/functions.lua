...
local function Message(Colour,Name,Message)
    if CLIENT then return pjaPrint(Message) end
    if SERVER then return pjaPrint(Message) end
...
local function Disconnect(Ply)
    if Disconnected:GetInt() == 0 then return end
        ...
        if SteamD:GetInt() == 0 then
            ...
            Message(team.GetColor(Ply:Team()),Ply:Nick()," disconnected. (" .. string.Replace(Ply:SteamID(),"STEAM_","") .. ")")
